...
namespace FormList
{
    public partial class UserMaster_T : Services.BaseChildForm
    {
        ...
        private void UserMaster_T_Load(object sender, EventArgs e)
        {
            ...
            _gridUtil.InitColumnGridUtil(myGrid2, "USERID",   "사용자ID", typeof(string),   DataGridViewContentAlignment.MiddleLeft,   100, true,  true);
            _gridUtil.InitColumnGridUtil(myGrid2, "USERNAME", "사용자명", typeof(string),   DataGridViewContentAlignment.MiddleLeft,   150, true,  true);
            _gridUtil.InitColumnGridUtil(myGrid2, "PW",       "비밀번호", typeof(string),   DataGridViewContentAlignment.MiddleLeft,   100, true,  true);
            _gridUtil.InitColumnGridUtil(myGrid2, "T_COUNT",  "실패횟수", typeof(string),   DataGridViewContentAlignment.MiddleRight,  100, true,  true);
            _gridUtil.InitColumnGridUtil(myGrid2, "DEPTCODE", "관리부서", typeof(string),   DataGridViewContentAlignment.MiddleLeft,   150, true,  true);
            _gridUtil.InitColumnGridUtil(myGrid2, "MAKEDATE", "등록일시", typeof(DateTime), DataGridViewContentAlignment.MiddleCenter, 200, false, true);
            _gridUtil.InitColumnGridUtil(myGrid2, "MAKER",    "생성자",   typeof(string),   DataGridViewContentAlignment.MiddleLeft,   150, false, true);
            _gridUtil.InitColumnGridUtil(myGrid2, "EDITDATE", "수정일시", typeof(DateTime), DataGridViewContentAlignment.MiddleCenter, 200, false, true);
            _gridUtil.InitColumnGridUtil(myGrid2, "EDITOR",   "수정자",   typeof(string),   DataGridViewContentAlignment.MiddleLeft,   150, false, true);

            #region < 콤보박스 세팅>
            Commons.GetCombo_Standard("DEPTCODE", cboDept1);
            #endregion
        }
        ...
        public override void DoInquire()
        {
            ...
            {
                // 그리드에 표현되어 있는 데이터 삭제
                ((DataTable)myGrid2.DataSource).Clear();  // 참조로 연결되어 있다.
                ...
                //Adapter.SelectCommand.Parameters.AddWithValue("@USERID", txtUserId1.Text);
                //Adapter.SelectCommand.Parameters.AddWithValue("@USERNAME", txtUserName1.Text);
                //Adapter.SelectCommand.Parameters.AddWithValue("@DEPTCODE", Convert.ToString(cboDept1.SelectedValue));


                //Params_ p1 = helper.CreateParameters("@USERID", txtUserId1.Text);
                //Params_ p2 = helper.CreateParameters("@USERNAME", txtUserName1.Text);
                //Params_ p3 = helper.CreateParameters("@DEPTCODE", Convert.ToString(cboDept1.SelectedValue));
                ...
                DataTable dtTemp = helper.FillTable("SP_UserMaster_S1", CommandType.StoredProcedure,
                                                    helper.CreateParameters("@USERID", txtUserId1.Text), // 파람스 배열 만들어야함 DBHelper ㄱㄱ
                                                    helper.CreateParameters("@USERNAME", txtUserName1.Text),
                                                    helper.CreateParameters("@DEPTCODE", Convert.ToString(cboDept1.SelectedValue)));
                ...
                // 그리드 뷰에 데이터 삽입
                myGrid2.DataSource = dtTemp;

                #region < 그리드 콤보박스 셋팅 (부서)>

                Commons.SetGridComboBox(myGrid2, "DEPTCODE", "DEPTCODE");
        ...
        public override void DoNew()
        {
            ...
            Commons.SetGridComboBox(myGrid2, "DEPTCODE", "DEPTCODE",false);
        }

        public override void DoDelete()
        {
            // 삭제 버튼 클릭 시 로직
            if (myGrid2.Rows.Count == 0) return;
            //int iRowsIndex = myGrid2.CurrentRow.Index;    // 현재 선택한 행의 Index

            //// Heap 영역에 있는 그리드의 데이터 속성을 DataTable로 전달
            //DataTable dtTemp = (DataTable)myGrid2.DataSource;
        ...
        public override void DoSave()
        {
            ...
            try
            {
                ...
                DataTable dtTemp = ((DataTable)myGrid2.DataSource).GetChanges();  // 변경된 사항을 dtTemp에 넣어줌
